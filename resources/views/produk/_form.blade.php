    ...
    <div class="input-group">
        <select name="jenis_id" class="form-select @error('jenis_id') is-invalid @enderror">
            ...
            @foreach ($jenisList as $jenisItem)
                <option value="{{ $jenisItem->id }}"
                    {{ (old('jenis_id', $produk->jenis_id ?? '') == $jenisItem->id) ? 'selected' : '' }}>
                    {{ $jenisItem->nama }}
                </option>
            @endforeach
        </select>
        <a href="{{ route('jenis.create') }}" class="btn btn-outline-secondary" title="Tambah jenis baru">
